    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-5 py-8">
          ...
          <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">フォーム作成とバリデーション</h1>
          <p className="mt-3 max-w-3xl leading-7 text-slate-600">
            issue #10 と #11 をまとめて練習します。ここでは完成品を見るだけでなく、どのファイルの何行目付近に何を書くかを順番に確認します。
          </p>
        </div>
      </header>
      ...
      <section className="mx-auto grid max-w-6xl gap-6 px-5 py-10 lg:grid-cols-[0.62fr_0.38fr]">
        <div className="space-y-6">
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-black uppercase tracking-wider text-emerald-700">Issue #10</p>
            <h2 className="mt-2 text-2xl font-black">入力できるフォームを作る</h2>
          ...
          <article className="rounded-lg bg-slate-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-black">useState と onChange のヒント</h2>
            <p className="mt-2 text-sm font-semibold text-emerald-300">1行目、import、state、inputのつながりを見る</p>
...
/>`}</code>
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-black uppercase tracking-wider text-emerald-700">Issue #11</p>
            <h2 className="mt-2 text-2xl font-black">入力チェックを追加する</h2>
            <ol className="mt-5 space-y-3">
          ...
          <article className="rounded-lg bg-slate-950 p-6 text-white shadow-sm">
            <h2 className="text-xl font-black">バリデーションのヒント</h2>
            <p className="mt-2 text-sm font-semibold text-emerald-300">未入力を先に確認し、その後メール形式を見る</p>
            <pre className="mt-5 overflow-x-auto rounded-md bg-slate-900 p-4 text-sm leading-6 text-slate-100 ring-1 ring-slate-700">
        ...
        <aside className="space-y-6 lg:sticky lg:top-6 lg:self-start">
          <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <form className="mt-6 space-y-4">
              ...
              <p className="rounded-md bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700 ring-1 ring-rose-200">
          ...
          <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 text-sm font-semibold leading-6 text-slate-700">
              <li>空欄で送信したら未入力エラーが出る。</li>
              <li>@ がないメールなら形式エラーが出る。</li>
              <li>正しい入力にしたらエラーが消える。</li>
              <li>入力欄に文字を打っても消えない。</li>
            </ul>
          </article>

          <div className="grid gap-3">
            <Link href="/tutorials/issue-10" className="rounded-md border border-slate-300 bg-white px-4 py-3 text-center text-sm font-bold hover:bg-slate-100">
              issue #10 の詳細へ
            </Link>
            <Link href="/tutorials/issue-11" className="rounded-md bg-slate-950 px-4 py-3 text-center text-sm font-bold text-white hover:bg-slate-800">
              issue #11 の詳細へ
            </Link>
          </div>
        </aside>
