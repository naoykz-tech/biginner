    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-6xl px-5 py-8">
          ...
          <h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">ポートフォリオ公開準備</h1>
          <p className="mt-3 max-w-3xl leading-7 text-slate-600">
            issue #15、#16、#20 を参考に、学習用プロジェクトを第三者へ説明できる成果物へ整えます。
          </p>
        </div>
      </header>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <article className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm lg:col-span-2">
            ...
            <p className="mt-3 leading-7 text-slate-600">
              READMEはただの説明書ではなく、どんな目的で作り、何を学び、どこまで実装したかを伝える資料です。
            </p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-5">
            {releaseItems.map((item, index) => (
              <div key={item} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                <span className="flex h-9 w-9 items-center justify-center rounded-md bg-white text-sm font-black text-slate-950 ring-1 ring-slate-200">
                  {index + 1}
                </span>
